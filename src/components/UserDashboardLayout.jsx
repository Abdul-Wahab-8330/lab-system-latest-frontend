...
const UserDashboardLayout = () => {
  ...
  const menuItems = [
    ...
    { id: 'all patients', icon: Users, label: 'All Patients', link:'/user/all-patients' },
    { id: 'payments', icon: DollarSign, label: 'Payments', link:'/user/payments' },
    { id: 'results', icon: Microscope, label: 'Results', link:'/user/results' },
    { id: 'reg reports', icon: FileText, label: 'Reg. Reports', link:'/user/patients' },
    { id: 'final reports', icon: ClipboardList, label: 'Final Reports', link:'/user/result-print' },
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-64' : 'w-19'} bg-white shadow-lg transition-all duration-300 ease-in-out flex flex-col`}>
        {/* Logo/Header */}
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center justify-center">
            <Link onClick={() => setActiveTab('Dashboard')} to='/user/dashboard' className="flex items-center space-x-2">
              {/* <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
                <Microscope className="w-5 h-5 text-white" />
              </div> */}
              <div>
                <img src={lablogo} alt="" className='w-15' />
              </div>
              {sidebarOpen && (
        ...
        {/* User Profile */}
        <div className={`p-4 border-t border-gray-200 ${sidebarOpen ? 'block' : 'hidden'}`}>
          <DropdownMenu open={open} onOpenChange={setOpen}>
            ...
            <DropdownMenuContent className="cursor-pointer w-56 bg-gray-50 border border-gray-300 shadow-lg">
              ...
              {user?.role === 'admin' ? (
                <DropdownMenuItem className='hover:bg-gray-100 cursor-pointer' onClick={() => console.log("Customize clicked")}>
        ...
        <header className="bg-white shadow-sm border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              {/* Export Button */}
              {user?.role == 'admin' && <button className="flex items-center space-x-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors duration-200">
              ...
              {/* Add New Button */}
              {user?.role=='admin' ? 
              <Link to='' className="flex items-center space-x-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors duration-200 shadow-sm">
              ...
              <div className='flex text-gray-600 gap-2'>
                <div className='bg-gray-50 rounded-xl py-2 px-2 text-sm'>{user?.role == 'user' ? <span className='flex gap-2 '><User size={17}/> Lab User</span>:<span className='flex gap-2'><Crown className='text-amber-400'  size={17}/> Admin</span>}</div>
                <UserAvatar/>
              </div>
            </div>
          </div>
