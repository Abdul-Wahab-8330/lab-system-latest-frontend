...
function UserDashboard() {
  ...
  const completedToday = addedPatients.filter(p => {
    ...
    const originalDate = new Date(p.updatedAt);
  ...
  return completedToday.length;
    ...
    // Assuming critical results are those with high-value tests or urgent cases
  ...
  // Convert patients data to recent tests format
  const recentTests = patients.slice().reverse().slice(0, 5).map(patient => ({
    id: patient.refNo,
    ...
    <main className="flex-1 overflow-y-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {dashboardStats.map((stat, index) => {
          ...
          return (
            <div key={index} className="bg-white rounded-xl shadow-md p-6 border border-gray-100 hover:shadow-md transition-shadow duration-200">
